...
function addObserverIfDesiredNodeAvailable(contents) {
  ...
  if (MutationObserver) {
    ...
    const observer = new MutationObserver(function (list, observer) {
      ...
      if (
        (waterImageNode &&
          !waterImageNode.getAttribute("style").includes("background-image")) ||
        !waterImageNode
      ) {
        // 避免一直触发
        observer.disconnect();
...
function createCanvasToUrl(contents) {
  ...
  ctx.fillText(
    userName + tele,
    parseFloat(styles.width) / 3,
    parseFloat(styles.height) / 3
  );
...
class Home extends Component {
  ...
  convert = () => {
    convertToImage(document.getElementById("homeContainer"), {
    ...
    return (
      <div className="home" id="home">
        <div id="homeContainer">
          <button onClick={() => downloadPdf("pdfContainer", "文件2")}>
            （必须位于顶部）下载pdf
          </button>
